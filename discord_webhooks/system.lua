...
local function internalSendToURL(name, url, content, callBackEvent)
	local postData
	...
	elseif type(content) == "table" then
		...
		if not valid then
			return false, "Invalid embed: "..tostring(additional).." (argument 2)"
		end
		postData = {
			["embeds"] = {additional}
		}
	...
	if not request then
		return false, "fetchRemote failed"
	end
	return request
end

-- Exported function
function send(name, content, callBackEvent)
	...
	return internalSendToURL(name, url, content, callBackEvent)
end
addEventHandler(RES_NAME..":send", root, send, false)
...
-- Exported function
function sendToURL(url, content, callBackEvent)
	if type(url) ~= "string" then
		return false, "Bad argument @ 'sendToURL' [Expected string at argument 1, got "..type(url).."]"
	end
	if not isValidURL(url) then
		outputDebugString("["..RES_NAME.."] sendToURL: URL may not be valid: "..url, 2)
	end
	...
	return internalSendToURL(foundName, url, content, callBackEvent)
end
addEventHandler(RES_NAME..":sendToURL", root, sendToURL, false)

addEventHandler("onResourceStart", resourceRoot, function()
	if type(WEB_HOOKS) ~= "table" then
		outputServerLog("["..RES_NAME.."] WEB_HOOKS is not a table. Please check your config.lua file.")
		return cancelEvent()
	end
	for name, url in pairs(WEB_HOOKS) do
		if type(name) ~= "string" then
			outputServerLog("["..RES_NAME.."] WEB_HOOKS contains a non-string key. Please check your config.lua file.")
			return cancelEvent()
		end
		if type(url) ~= "string" then
			outputServerLog("["..RES_NAME.."] WEB_HOOKS contains a non-string value. Please check your config.lua file.")
			return cancelEvent()
		end
	end
	if type(SETTING_DISABLE) == "string"  then
		...
		if (settingValue == "true" or settingValue == "1") then
			if (DEBUG_ON) then
				outputServerLog("["..RES_NAME.."] Webhooks are disabled by the server's settings registry.")
			end
			-- Disable the exported functions
			send = function() end
			sendToURL = function() end
			return
		end
	end
	if (DEBUG_ON) then
		outputServerLog("["..RES_NAME.."] Initialization complete.")
	end
end, false)
