...
local function internalSendRequest(name, url, content, callBackEvent)
	...
	if (LOG_INFO_DEBUG) then
		outputDebugString("["..RES_NAME.."] Sending to URL '"..url.."' with data:", 3)
...
function send(...)
	...
	if (not result) and (LOG_ERRORS_DEBUG == true) then
		outputDebugString("["..RES_NAME.."] "..tostring(reason), 1)
	end
...
addEventHandler("onResourceStart", resourceRoot, function()
	...
	if type(SETTING_DISABLE) == "string"  then
		...
		if (settingValue == "true" or settingValue == "1") then
			if (LOG_INFO_DEBUG) then
				outputDebugString("["..RES_NAME.."] Webhooks are disabled by the server's settings registry.", 3)
			end
	...
	if (LOG_INFO_DEBUG) then
		outputDebugString("["..RES_NAME.."] Initialization complete ("..#WEB_HOOKS.." webhooks declared).", 3)
	end
end, false)
